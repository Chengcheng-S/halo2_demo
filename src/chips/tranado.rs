...
pub struct TornadoChip<F> {
    pub config: TronadoConfig,
    _marker: PhantomData<F>,
}

impl<F: PrimeField> TornadoChip<F> {
    pub fn construct(config: TronadoConfig) -> Self {
        Self {
            config,
            _marker: PhantomData,
        }
    ...
    pub fn compute_hash(
        ...
        let result_hash_cell = hash_chip.hash(layouter.namespace(|| "hash values"), left, right)?;
        Ok(result_hash_cell)
    }
}
