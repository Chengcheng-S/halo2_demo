...
impl<F:PrimeField>Circuit<F> for TornadoCircuit<F>{
    ...
    fn synthesize(&self, config: Self::Config, mut layouter: impl Layouter<F>) -> Result<(), Error> {
        ...
        let nullifier_hash_cell = torndao_chip.compute_hash(layouter.namespace(|| "get nullifier hash"),self.nullifier,self.nullifier )?;
        ...
        let commit_hash_cell = torndao_chip.compute_hash(layouter.namespace(|| "get commit hash"),self.nullifier,self.secret)?;
